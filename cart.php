...
$stmt = $conn->prepare("SELECT COUNT(*) as total_postulations_entreprise FROM utilisateur WHERE type_utilisateur = 'entreprise' AND formation_id IS NOT NULL");
$stmt->execute();
...
$stmt = $conn->prepare("SELECT COUNT(*) as total_postulations_etudiant FROM utilisateur WHERE type_utilisateur = 'etudiant' AND formation_id IS NOT NULL");
$stmt->execute();
$result_postulation_etudiant = $stmt->get_result();
$data_postulation_etudiant = $result_postulation_etudiant->fetch_assoc();
$total_postulations_etudiant = $data_postulation_etudiant['total_postulations_etudiant'];

$stmt = $conn->prepare("SELECT COUNT(*) as total_postulations_formateur FROM utilisateur WHERE type_utilisateur = 'formateur' AND cv IS NOT NULL");
$stmt->execute();
...
// Récupérer les coordonnées des étudiants, des formateurs et des entreprises
$stmt = $conn->prepare("SELECT utilisateur.*, formation.nom as formation_nom FROM utilisateur LEFT JOIN formation ON utilisateur.formation_id = formation.id WHERE type_utilisateur = 'etudiant'");
$stmt->execute();
$result_etudiants = $stmt->get_result();

$stmt = $conn->prepare("SELECT * FROM utilisateur WHERE type_utilisateur = 'formateur'");
$stmt->execute();
$result_formateurs = $stmt->get_result();

$stmt = $conn->prepare("SELECT utilisateur.*, formation.nom as formation_nom FROM utilisateur LEFT JOIN formation ON utilisateur.formation_id = formation.id WHERE type_utilisateur = 'entreprise'");
$stmt->execute();
...
?><!doctypehtml><html lang=fr><meta charset=UTF-8><meta content="width=device-width,initial-scale=1"name=viewport><title>Statistiques des Tables</title><link href=https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css rel=stylesheet><link href=./assets/css/stylec.css rel=stylesheet><link href=https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css rel=stylesheet><script src=https://cdn.jsdelivr.net/npm/chart.js></script><style>.stats-container{margin-top:50px}.card{border:1px solid #ddd;border-radius:10px;box-shadow:0 4px 8px rgba(0,0,0,.1)}.card-body{text-align:center}.card-title{font-size:1.5rem;color:#007bff}.card-text{font-size:2rem;font-weight:700}.delete-btn{background-color:#ff4d4d;color:#fff;border:none;border-radius:5px;padding:8px 12px;cursor:pointer;font-size:14px;transition:background-color .3s ease}.delete-btn:hover{background-color:#ff1a1a}table{width:100%;border-collapse:collapse}table td,table th{border:1px solid #ddd;padding:8px;text-align:center}table th{background-color:#f4f4f4}header,header *{text-decoration:none;text-align:center}</style><div class="container stats-container"><div class=text-center><a href=admin.php class=back-btn>Retour</a></div><style>.back-btn{display:inline-block;padding:10px 20px;font-size:18px;font-weight:700;color:#fff;background:linear-gradient(45deg,#009fb0,#00d4ff);border:none;border-radius:50px;text-align:center;text-decoration:none;transition:background .3s ease,transform .3s ease}.back-btn:hover{background:linear-gradient(45deg,#0056b3,#00a3cc);transform:scale(1.05)}.text-center{text-align:center;margin-top:20px}</style><br><h1 class=heading>Statistiques/Informations</h1><div class="justify-content-center row"><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Nombre Total de Postulations Entreprises</h5><p class=card-text><?php echo $total_postulations_entreprise; ?></div></div></div><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Nombre Total de Postulations Étudiants</h5><p class=card-text><?php echo $total_postulations_etudiant; ?></div></div></div><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Nombre Total de Postulations Formateurs</h5><p class=card-text><?php echo $total_postulations_formateur; ?></div></div></div><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Formations Disponibles</h5><p class=card-text><?php echo $total_formations_disponibles; ?></div></div></div><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Formations Indisponibles</h5><p class=card-text><?php echo $total_formations_indisponibles; ?></div></div></div><div class=col-md-3><div class=card><div class=card-body><h5 class=card-title>Nombre Total de Thèmes</h5><p class=card-text><?php echo $total_themes; ?></div></div></div></div><div class="justify-content-center row mt-5"><div class=col-md-8><div class=card><div class=card-body><h5 class=card-title>Statistiques Globales</h5><canvas id=statistiquesGraph></canvas></div></div></div></div></div><script>var totalPostulationsEntreprise=<?php echo $total_postulations_entreprise; ?>,totalPostulationsEtudiant=<?php echo $total_postulations_etudiant; ?>,totalPostulationsFormateur=<?php echo $total_postulations_formateur; ?>,totalFormationsDisponibles=<?php echo $total_formations_disponibles; ?>,totalFormationsIndisponibles=<?php echo $total_formations_indisponibles; ?>,totalThemes=<?php echo $total_themes; ?>,ctx=document.getElementById("statistiquesGraph").getContext("2d"),statistiquesGraph=new Chart(ctx,{type:"bar",data:{labels:["Postulations Entreprises","Postulations Étudiants","Postulations Formateurs","Formations Disponibles","Formations Indisponibles","Thèmes"],datasets:[{label:"Nombre",data:[totalPostulationsEntreprise,totalPostulationsEtudiant,totalPostulationsFormateur,totalFormationsDisponibles,totalFormationsIndisponibles,totalThemes],backgroundColor:["#007bff","#dc3545","#28a745","#ffc107","#6f42c1","#17a2b8"],borderColor:["#0056b3","#c82333","#218838","#e0a800","#563d7c","#138496"],borderWidth:1}]},options:{responsive:!0,plugins:{legend:{position:"top"},tooltip:{callbacks:{label:function(t){return t.raw}}}},scales:{y:{beginAtZero:!0,title:{display:!0,text:"Nombre"}}}}})</script><script src=https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js></script><div class=container><br><h1 class=heading>Coordonnées des Postulations</h1><div class=btn-group aria-label="Basic example"role=group><button class="btn btn-primary"type=button onclick='showTable("entreprises")'>Entreprises</button> <button class="btn btn-secondary"type=button onclick='showTable("etudiants")'>Étudiants</button> <button class="btn btn-success"type=button onclick='showTable("formateurs")'>Formateurs</button></div><section class=shopping-cart><div class=table-responsive id=entreprisesTable><h2>Entreprises</h2><table><thead><tr><th>ID<th>Nom<th>Email<th>Numéro Téléphone<th>Formation<th>Image<th>Vue<th>Action<tbody><?php
                        if ($result_entreprises->num_rows > 0) {
                            ...
                            ?><tr><td colspan=8>Aucune entreprise trouvée.</tr><?php
                        }
                        ?></table></div><div class=table-responsive id=etudiantsTable style=display:none><h2>Étudiants</h2><table><thead><tr><th>ID<th>Nom<th>Email<th>Numéro Téléphone<th>Formation<th>Image<th>Vue<th>Action<tbody><?php
                        ...
                        ?></table></div></section></div><script>function showTable(e){document.getElementById("entreprisesTable").style.display="entreprises"===e?"block":"none",document.getElementById("etudiantsTable").style.display="etudiants"===e?"block":"none",document.getElementById("formateursTable").style.display="formateurs"===e?"block":"none"}</script><script src=https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js></script>